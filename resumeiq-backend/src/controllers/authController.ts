...
export const login = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const accessToken = generateAccessToken({ id: user._id as string, role: user.role, email: user.email });
...
export const refreshSession = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    try {
      ...
      const newAccessToken = generateAccessToken({ id: user._id as string, role: user.role, email: user.email });
      const newRefreshToken = generateRefreshToken({ id: user._id as string });
